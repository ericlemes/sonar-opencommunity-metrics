...
public class ComplexFunctionsLoCMeasureComputer implements MeasureComputer {

  @Override
  public MeasureComputerDefinition define(MeasureComputerDefinitionContext context) {
    return context.newDefinitionBuilder()
      .setOutputMetrics(OpenCommunityMetrics.COMPLEX_FUNCTIONS_LOC.key())
      .build();   
  }
  ...
  public void compute(MeasureComputerContext context) {               
    if (context.getComponent().getType() == Component.Type.PROJECT)
      context.addMeasure(OpenCommunityMetrics.COMPLEX_FUNCTIONS_LOC.key(), sum);
    else if (context.getComponent().getType() == Component.Type.FILE){
      Measure m = context.getMeasure(OpenCommunityMetrics.COMPLEX_FUNCTIONS_LOC.key());
      if (m != null)
        sum += m.getIntValue();
    }    
  }
