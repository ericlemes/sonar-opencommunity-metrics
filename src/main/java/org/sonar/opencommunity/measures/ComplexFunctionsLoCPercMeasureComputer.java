...
public class ComplexFunctionsLoCPercMeasureComputer implements MeasureComputer {

  @Override
  public MeasureComputerDefinition define(MeasureComputerDefinitionContext context) {
    return context.newDefinitionBuilder()
      .setInputMetrics(OpenCommunityMetrics.LOC_IN_FUNCTIONS.key(), OpenCommunityMetrics.COMPLEX_FUNCTIONS_LOC.key())
      .setOutputMetrics(OpenCommunityMetrics.COMPLEX_FUNCTIONS_LOC_PERC.key())
      .build(); 
  }

  private int totalLoCInFunctions = 0;
  private int totalLoCInComplexFunctions = 0;

  @Override
  public void compute(MeasureComputerContext context) {
    if (context.getComponent().getType() == Component.Type.PROJECT){
      context.addMeasure(OpenCommunityMetrics.COMPLEX_FUNCTIONS_LOC_PERC.key(), calculatePercentual(totalLoCInComplexFunctions, totalLoCInFunctions));
    }
      ...
      m = context.getMeasure(OpenCommunityMetrics.COMPLEX_FUNCTIONS_LOC.key());
      if (m != null)
        totalLoCInComplexFunctions += m.getIntValue();
    }    
  }
