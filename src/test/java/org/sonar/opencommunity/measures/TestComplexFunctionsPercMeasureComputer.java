...
public class TestComplexFunctionsPercMeasureComputer {
  private ComplexFunctionsPercMeasureComputer measureComputer;
  ...
  public void setUp(){
    ...
    this.measureComputer = new ComplexFunctionsPercMeasureComputer();
  ...
  public void testWhenDefiningShouldReturnCorrectDefinition(){
    ...
    verify(this.definitionContextBuilder).setInputMetrics(OpenCommunityMetrics.COMPLEX_FUNCTIONS.key(), CoreMetrics.FUNCTIONS_KEY);
    verify(this.definitionContextBuilder).setOutputMetrics(OpenCommunityMetrics.COMPLEX_FUNCTIONS_PERC.key());
    verify(this.definitionContextBuilder).build();
  }
  
  @Test
  public void testWhenCallingComputeShouldComputeCorrectly(){        
   Component file = MeasureComputerTestHelper.setupComponentAndIntMeasure(context, Component.Type.FILE, 5, OpenCommunityMetrics.COMPLEX_FUNCTIONS.key());    
    MeasureComputerTestHelper.addIntMeasure(context, file, 10, CoreMetrics.FUNCTIONS.key());
    this.measureComputer.compute(context);
    
    Component file2 = MeasureComputerTestHelper.setupComponentAndIntMeasure(context, Component.Type.FILE, 2, OpenCommunityMetrics.COMPLEX_FUNCTIONS.key());    
    ...
    verify(context).addMeasure(OpenCommunityMetrics.COMPLEX_FUNCTIONS_PERC.key(), 35.0d);
   }
  ...
  public void testWhenCallingComputeWithFileWithNoFunctionsShouldReturn0(){
    ...
    verify(context).addMeasure(OpenCommunityMetrics.COMPLEX_FUNCTIONS_PERC.key(), 0d);
  }    
}
