...
public class TestComplexFunctionsMeasureComputer {
  private ComplexFunctionsMeasureComputer measureComputer;
  ...
  public void setUp(){
    ...
    this.measureComputer = new ComplexFunctionsMeasureComputer();
  ...
  public void testWhenDefiningShouldReturnCorrectDefinition(){
    ...
    verify(this.definitionContextBuilder).setOutputMetrics(OpenCommunityMetrics.COMPLEX_FUNCTIONS.key());
    verify(this.definitionContextBuilder).build();
  }
  
  @Test
  public void testWhenCallingComputeShouldComputeSumCorrectly(){        
    MeasureComputerTestHelper.setupComponentAndIntMeasure(context, Component.Type.FILE, 3, OpenCommunityMetrics.COMPLEX_FUNCTIONS.key());    
    this.measureComputer.compute(context);
    
    MeasureComputerTestHelper.setupComponentAndIntMeasure(context, Component.Type.FILE, 2, OpenCommunityMetrics.COMPLEX_FUNCTIONS.key());    
    this.measureComputer.compute(context);
    
    MeasureComputerTestHelper.setupProject(context);
    this.measureComputer.compute(context);
    
    verify(context).addMeasure(OpenCommunityMetrics.COMPLEX_FUNCTIONS.key(), 5);
  }
