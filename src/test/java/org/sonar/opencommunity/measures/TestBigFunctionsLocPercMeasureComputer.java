...
public class TestBigFunctionsLocPercMeasureComputer {
  private BigFunctionsLoCPercMeasureComputer measureComputer;
  ...
  public void setUp(){
    ...
    this.measureComputer = new BigFunctionsLoCPercMeasureComputer();
  ...
  public void testWhenDefiningShouldReturnCorrectDefinition(){
    ...
    verify(this.definitionContextBuilder).setInputMetrics(OpenCommunityMetrics.LOC_IN_FUNCTIONS.key(), OpenCommunityMetrics.BIG_FUNCTIONS_LOC.key());
    verify(this.definitionContextBuilder).setOutputMetrics(OpenCommunityMetrics.BIG_FUNCTIONS_LOC_PERC.key());
  ...
  public void testWhenCallingComputeShouldComputeCorrectly(){        
    ...
    MeasureComputerTestHelper.addIntMeasure(context, file, 50, OpenCommunityMetrics.BIG_FUNCTIONS_LOC.key());
    ...
    MeasureComputerTestHelper.addIntMeasure(context, file2, 5, OpenCommunityMetrics.BIG_FUNCTIONS_LOC.key());
    this.measureComputer.compute(context);
    
    MeasureComputerTestHelper.setupProject(context);
    this.measureComputer.compute(context);
    
    verify(context).addMeasure(OpenCommunityMetrics.BIG_FUNCTIONS_LOC_PERC.key(), 44.0d);
   }
  ...
  public void testWhenCallingComputeWithFileWithNoLoCInFunctionsShouldReturn0(){
    ...
    MeasureComputerTestHelper.addIntMeasure(context, file, 0, OpenCommunityMetrics.BIG_FUNCTIONS_LOC.key());   
    this.measureComputer.compute(context);
    
    MeasureComputerTestHelper.setupProject(context);
    this.measureComputer.compute(context);
    
    verify(context).addMeasure(OpenCommunityMetrics.BIG_FUNCTIONS_LOC_PERC.key(), 0d);
  }    
}
