...
public class TestComplexFunctionsLoCMeasureComputer {
  private ComplexFunctionsLoCMeasureComputer measureComputer;
  ...
  public void setUp(){
    ...
    this.measureComputer = new ComplexFunctionsLoCMeasureComputer();
  ...
  public void testWhenDefiningShouldReturnCorrectDefinition(){
    ...
    verify(this.definitionContextBuilder).setOutputMetrics(OpenCommunityMetrics.COMPLEX_FUNCTIONS_LOC.key());
    verify(this.definitionContextBuilder).build();
  }
  
  @Test
  public void testWhenCallingComputeShouldComputeSumCorrectly(){        
    MeasureComputerTestHelper.setupComponentAndIntMeasure(context, Component.Type.FILE, 15, OpenCommunityMetrics.COMPLEX_FUNCTIONS_LOC.key());    
    this.measureComputer.compute(context);
    
    MeasureComputerTestHelper.setupComponentAndIntMeasure(context, Component.Type.FILE, 20, OpenCommunityMetrics.COMPLEX_FUNCTIONS_LOC.key());    
    this.measureComputer.compute(context);
    
    MeasureComputerTestHelper.setupProject(context);
    this.measureComputer.compute(context);
    
    verify(context).addMeasure(OpenCommunityMetrics.COMPLEX_FUNCTIONS_LOC.key(), 35);
  }
  ...
  public void testWhenCallingComputeWithFileWithNoMeasureShouldNotThrow(){
    MeasureComputerTestHelper.setupComponentAndIntMeasure(context, Component.Type.FILE, 42, OpenCommunityMetrics.COMPLEX_FUNCTIONS_LOC.key());    
    when(this.context.getMeasure(OpenCommunityMetrics.COMPLEX_FUNCTIONS_LOC.key())).thenReturn(null);
    this.measureComputer.compute(context);
